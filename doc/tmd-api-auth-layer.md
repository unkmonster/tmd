...
| **双模式认证** | 同时支持 JWT 会话令牌和原始 API Key，客户端可逐步迁移 |
...
├─────────────────────┤
│ authMiddleware      │  ← 双模式认证：JWT + 原始 API Key
├─────────────────────┤
...
| 有 API Key 无 JWT | `Bearer <api_key>` | JWT 失败 → Key 匹配 → 放行 | ✅ 向后兼容 |
| 有有效 JWT | `Bearer <jwt>` | JWT 验证通过 → 放行 | ✅ 推荐模式 |
| JWT 过期 | `Bearer <expired_jwt>` | JWT 失败（过期）→ Key 不匹配 → 401 `expired` | 客户端 refresh |
| JWT 签名错误 | `Bearer <tampered_jwt>` | JWT 失败（签名）→ Key 不匹配 → 401 `invalid` | 客户端弹认证框 |
| API Key 变更后旧 JWT | `Bearer <old_jwt>` | JWT 失败（签名，密钥已变）→ 401 `invalid` | 客户端弹认证框 |
...
1. **JWT 会话令牌**（`localStorage.tmd_jwt_token`）— 首选，签发后 1 小时有效
2. **原始 API Key**（`localStorage.tmd_api_key`）— 回退，用于兼容旧主题或 SSE 降级
...
### 3. JWT 与 API Key 的安全对比

| 方面 | 原始 API Key | JWT Session Token |
|------|-------------|-------------------|
| 传输频率 | 每次请求完整 Key | 每次请求（1h 有效） |
| 泄露影响 | Key 永久有效，需修改 `conf.yaml` | 1 小时有效，过期自动失效 |
| 撤销方式 | 修改 `conf.yaml` | 等待过期或修改 API Key |
| 签名密钥 | 不适用 | 从 API Key 派生（不直接使用） |
| 网络暴露 | 每次请求明文 | 每次请求明文，但有时效 |
...
| 文件 | 说明 |
|------|------|
| `internal/api/middleware.go` | `authMiddleware()` 双模式认证实现、`isPublicPath()` 白名单、`extractBearerToken()` token 提取、`writeAuth401()` 401 响应 |
