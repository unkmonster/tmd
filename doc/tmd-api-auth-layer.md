...
`api_key` 在系统设置的配置编辑器中以 **password** 类型显示（值被脱敏，如 `abc•••xyz`），归属于 `security` 分组。通过 Web UI 修改后需要重启 Server 生效。
...
认证对话框是一个浮动抽屉（Drawer）组件，位于 `index.html` 中：

- **触发条件**：任何 API 调用返回 HTTP 401
- **输入**：密码类型输入框（可切换显示/隐藏）
- **确认**：保存到 `localStorage` 并刷新页面
- **样式**：复用已有 Drawer 组件的 CSS（`drawer-overlay` + `drawer`）

### 系统设置 → 安全标签页

在系统设置页面新增了 **🔐 安全** 标签页，提供：

| 功能 | 说明 |
|------|------|
| **API Key 输入框** | password 类型，已有 key 则预填充 |
| **显示/隐藏切换** | 👁️ 按钮切换密码可见性 |
| **保存到本地** | 写入 `localStorage`，刷新后自动携带 |
| **测试连接** | 向 `/api/v1/tasks` 发送认证请求验证 key 有效性 |
| **清除** | 从 `localStorage` 移除 key |

### 前端存储机制

- **存储位置**：`localStorage`（键名：`tmd_api_key`）
- **生命周期**：持久化存储，关闭浏览器不清除
- **自动携带**：所有 API 请求通过 `api.request()` 自动从 `localStorage` 读取并注入 `Authorization` 头
- **清除方式**：系统设置 → 安全 → 清除按钮，或手动 `localStorage.removeItem('tmd_api_key')`
...
认证层的 API Key 在 **Server 启动时读取**。修改配置后需要通过以下方式之一使新 Key 生效：

- 重启 TMD Server
- 发送 `POST /api/v1/server/shutdown` 后重新启动（注意：关闭后需手动启动）
...
| `internal/api/web/web1/app.js` | 前端认证集成：api.request()、SSE token、认证对话框 |
| `internal/api/web/web1/index.html` | 认证对话框 HTML 结构 |
