...
💡 **前端存储机制**：Key 存入浏览器 `localStorage`（键名 `tmd_api_key`），所有 API 请求自动从中读取并注入 `Authorization` 头。SSE 连接自动追加 `?token=` 参数。
...
- **传输安全**：公网部署强烈建议使用 HTTPS（Nginx/Caddy 反向代理终止 TLS）
- **Key 轮换**：定期更换 API Key
- **日志安全**：API Key **不会**出现在 TMD 请求日志中
- **已知限制**：仅支持 Bearer 方案，无多用户/角色/权限/Token 过期机制
...
> 🔓 **公开端点**：即使开启了 API Key 认证，此端点也无需认证（需要在请求体中提供 API Key）。
...
TMD 内置 API Key 认证层（自 v3.5 起）：

- **开启方式**：在 `conf.yaml` 中设置 `api_key`，或通过环境变量 `TMD_API_KEY`
- **认证方式**：`Authorization: Bearer <key>` 请求头（首选），SSE 端点回退 `?token=` 查询参数
- **公开路径**：Web UI 页面、静态文件、健康检查、主题切换免认证（SPA 兼容设计）
...
const sse = new EventSource('/api/v1/sse/tasks?token=' + encodeURIComponent(key));
